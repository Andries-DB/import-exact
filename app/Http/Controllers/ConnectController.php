...
class ConnectController extends Controller
{
    ...
    /*public function access_customer() {
        ...
        $result['customer'] = $this->get($access_token);
    ...
    public function getAccount($access_token) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, env('EXACT_API_URL') . '/api/v1/' . $division . '/crm/Accounts?$select=ID,Name,Website,Code');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function getGUIDCustomer($access_token, $code) {
        $division = env('DIVISION');
        $ch = curl_init();
    ...
    private function getBlockData($access_token) {
        $block = [];
        // Get only 19 bookings
        $allBookings = DB::table('aaa_exact_documenten')->get();

        // Een blok begint met een lijn met teller 1 en eindigt wanneer er een nieuwe lijn met teller 1 komt exclusief de laatste lijn
        // foreach($allBookings as $booking) {
        //     if($booking->teller == 1) {
        //         $block[] = $booking;
        //     } else {
        //         $block[count($block) - 1]->lines[] = $booking;
        //     }
        // }


        // Only get the bookings with klantcode 77238
        foreach($allBookings as $booking) {
            if($booking->klantcode == '77238') {
                if($booking->teller == 1) {
                    $block[] = $booking;
                } else {
                    $block[count($block) - 1]->lines[] = $booking;
